...
+ Anonymous functions: If the function is for single use within a looop, you could create an anonymous function. The anonymous function will only exist in the loops environment. Using the above example, the function inside `map(my_vector, function(x) x * -1)` will only exist inside `map`.

+ Storing various functions within a list: If you have multiple similar functions or you want to invoke multiple functions in a loop, you could store the functions as a list. For example, if you had two functions--`flip_sign_1 <- function(x) {x * -1}` and `flip_sign_2 <- function(x) {x * -2}`--you could store them in a list (e.g., `flip_sign <- list(flip_sign_1, flip_sign_2)`) and call them as needed (e.g., `flip_sign[[1]]` or `flip_sign[[2]]`).
...
my_mean(airquality$Ozone) #my function

mean(airquality$Ozone, na.rm = TRUE) #base function

# create vector with missing data

test <- c(1, 2, 3, 4, 5, NA, 8)

my_mean(test) #my function

mean(test, na.rm = TRUE) #base method
...
#num_cols_only(x) #does not print out correct msg?
#commented out because file will not knit if this runs
```
...
#describe_num_cols(x) #does not print out correct msg? 
#commented out because file will not knit if this runs
